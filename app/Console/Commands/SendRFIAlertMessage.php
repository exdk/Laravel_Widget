...
class SendRFIAlertMessage extends Command
{
   /**
    ...
    * @var string
    */
   protected $signature = 'rfi_alert:send';
   /**
    * The console command description.
    *
    * @var string
    */
   protected $description = 'Sending rfi mails, while RFI running';
   /**
   ...
   {
	   $rfis = Rfi::where("start", "<", Carbon::now()->format("Y-m-d H:i:00"))->where("finish", ">", Carbon::now()->format("Y-m-d H:i:59"))->get();
	   foreach($rfis as $rfi)
	   {
			$partner_emails = []; 
			...
			{
				$alert = UserAlert::create(["alert_text" => "Продолжение RFI", "alert_link" => "/admin/rfis/".$rfi->id]);
				$user_user_alert = [];
			...
			{
				$ans = $SMTPBZController->send_mail("Уведомление о продолжении RFI", $pe, $html);
			}
			...
			foreach($selected_transport_companies_employers as $e)
			{
				if($e->notifications_settings->rfi_end == 1)
				{
					$ans = $SMTPBZController->send_mail("Уведомление о продолжении RFI", $e->email, $html);
				}
			}
