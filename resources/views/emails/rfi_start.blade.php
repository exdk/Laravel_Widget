                                 ...
                                 <tbody>
                                    ...
                                    <tr>
                                       <td bgcolor="#F6F6F6" style="border-collapse:collapse;color:#333333;font-family:'calibri' , sans-serif;font-size:16px;padding:10px">
											<strong>Грузовладелец</strong>: {{$gruzowner_name}}<br>
											<strong>RFI</strong>: 
											<a href="https://admin.7rights.ru/admin/rfis/{{$rfi->id}}" target="_blank" style="color:#284a98;padding:0;text-decoration:underline" data-link-id="1" rel="noopener noreferrer">{{$rfi->title}} [ID: {{$rfi->id}}]</a>
											<a href="https://7rights.ru/admin/rfis/{{$rfi->id}}" target="_blank" style="color:#284a98;padding:0;text-decoration:underline" data-link-id="1" rel="noopener noreferrer">{{$rfi->title}} [ID: {{$rfi->id}}]</a>
											<br>
											<strong>Дата начала</strong>: {{$rfi->start}}<br>
											<strong>Дата окончания</strong>: {{$rfi->finish}}<br>
                                    ...
                                    <tr>
                                       <td bgcolor="#FFFFFF" style="border-collapse:collapse;color:#333333;font-family:'calibri' , sans-serif;font-size:16px;padding:10px">
                                          <div>Dear Sir or Madam,<br><br>the aforementioned preselection will end on {{$rfi->finish}}. We request that you submit an offer in Freight Procurement under 
										  ...
										  @if(!is_null($rfi->contact))
										  <br><br>Контактное лицо: {{$rfi->contact->fio}}</div>
                                          <!--<div>Компания: «»<br>-->
											<span class="ca6710e1bb46a547js-phone-number_mr_css_attr">{{$rfi->contact->mobile}}</span><br>
											<a href="http://e.mail.ru/compose/?mailto={{$rfi->contact->email}}" target="_blank" data-link-id="3" rel="noopener noreferrer">{{$rfi->contact->email}}</a>
										  </div>
										  @endif
										  @if(!is_null($rfi->contact_2))
										  <br><br>Контактное лицо: {{$rfi->contact_2->fio}}</div><br>
                                          <!--<div>Компания: «»<br>-->
											<span class="ca6710e1bb46a547js-phone-number_mr_css_attr">{{$rfi->contact_2->mobile}}</span><br>
											<a href="http://e.mail.ru/compose/?mailto={{$rfi->contact_2->email}}" target="_blank" data-link-id="4" rel="noopener noreferrer">{{$rfi->contact_2->email}}</a>
										  </div>
										  @endif
                                       </td>
                                    </tr>
