...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.partnergroup.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.partnergroups.update", [$partnergroup->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="title">{{ trans('cruds.partnergroup.fields.title') }}</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $partnergroup->title) }}">
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('partners') ? 'is-invalid' : '' }}" name="partners[]" id="partners" multiple>
                    @foreach($partners as $id => $partner)
                        <option value="{{ $id }}" {{ (in_array($id, old('partners', [])) || $partnergroup->partners->contains($id)) ? 'selected' : '' }}>{{ $partner }}</option>
                    @endforeach
                </select>
