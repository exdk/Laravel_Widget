...
class SendRFIEndMessage extends Command
{
   /**
    ...
    * @var string
    */
   protected $signature = 'send_rfi_end_alerts:send';
   /**
    * The console command description.
    *
    * @var string
    */
   protected $description = 'Sending rfi mails on finish date';
   /**
   ...
   {
	   $rfis = Rfi::where("finish", ">=", Carbon::now()->format("Y-m-d H:i:00"))->where("finish", "<=", Carbon::now()->format("Y-m-d H:i:59"))->get();
	   foreach($rfis as $rfi)
	   {
			$partner_emails = []; 
			...
			{
				$alert = UserAlert::create(["alert_text" => "Окончание RFI", "alert_link" => "/admin/rfis/".$rfi->id]);
				$user_user_alert = [];
			...
			{
				$ans = $SMTPBZController->send_mail("Уведомление об окончании RFI", $pe, $html);
			}
			...
			foreach($selected_transport_companies_employers as $e)
			{
				if($e->notifications_settings->rfi_end == 1)
				{
					$ans = $SMTPBZController->send_mail("Уведомление об окончании RFI", $e->email, $html);
				}
			}
