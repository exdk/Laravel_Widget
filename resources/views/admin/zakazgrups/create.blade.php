...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.zakazgrup.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.zakazgrups.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('zakazperevozs') ? 'is-invalid' : '' }}" name="zakazperevozs[]" id="zakazperevozs" multiple>
                    @foreach($zakazperevozs as $id => $zakazperevoz)
                        <option value="{{ $id }}" {{ in_array($id, old('zakazperevozs', [])) ? 'selected' : '' }}>{{ $zakazperevoz }}</option>
                    @endforeach
                </select>
