...
<head>
    ...
    <script src="https://api-maps.yandex.ru/2.1/?lang=ru_RU&amp;apikey=af458691-7d08-476e-bd7b-1d2fb6e50486" type="text/javascript"></script>
...
<ul class=\"navbar-nav ml-auto\">     
                <li class=\"nav-item\" style='margin-right:10px;'>
                    <a href=\"mies\">Мои грузы</a>
                </li>
                <li class=\"nav-item\" style='margin-right:10px;'>
                    <a href=\"zakaznagruzs\"><b>Все грузы</b></a>
                </li>
                <li class=\"nav-item\" style='margin-right:10px;'>
                    <a href=\"drafts\">Черновики</a>
                </li>
                <li class=\"nav-item\" style='margin-right:10px;'>
                    <a href=\"archives\">Архив</a>
                </li>
                <li class=\"nav-item\" style='margin-right:10px;'>
                    <a href=\"favouritels\">Избранное</a>
                </li>
                <li class=\"nav-item\" style='margin-right:10px;'>
                    <a href=\"offergs\">Согласование</a>
                </li>
</ul>
        ";
    }
@endphp
            ...
            @endif
                ...
                <li class="nav-item dropdown notifications-menu">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        <a class="dropdown-item" href="/admin/users/{{Auth::user()->id}}/edit" rel="noopener noreferrer">Профиль</a>
                        <a class="dropdown-item" href="/profile/password" rel="noopener noreferrer">Изменить пароль</a>
                        <a class="dropdown-item" href="#" onclick="event.preventDefault(); document.getElementById('logoutform').submit();">Выйти из системы</a>
                        <a class="dropdown-item" href="/admin/contacts">Поддержка</a>
                    </div>
                </li>
                <!--<li class="nav-item">
                    <a class="nav-link"href="http://admin.7rights.ru/profile/password"><i class="fa fa-user"></i>&nbsp;{{ Auth::user()->name }}/{{ Auth::user()->team_id }}</a>
                </li>-->
            </ul>
    ...
    <script src="{{ asset('js/provider.js') }}" type="text/javascript"></script>
  ...
  $.extend(true, $.fn.dataTable.defaults, {
    ...
    buttons: [
      ...
      },
      {
        extend: 'colvis',
        className: 'btn-primary',
        text: 'Видимость колонок 2',
        exportOptions: {
          columns: ':visible'
        },
        action: function(e, dt) {
          $('#dt_columns_custom_hide').modal('show');
        }
      }
    ]
  });
    ...
    @yield('scripts')
     <script src="https://snipp.ru/cdn/maskedinput/jquery.maskedinput.min.js"></script>
            <script>
                ...
                $('.time').mask('99:99');
                $('.nomervu').mask('9999 999999');
                $('.pa_ty').mask('99 99');
                $('.pa_nomer').mask('999999');
                $('.pfr').mask('999 999 999 99');
                $('.inn').mask('9999 9999 9999');
                $.mask.definitions['Z'] = "[АВЕКМНОРСТУХавекмнорстухABEKMHOPCTYXabekmhopctyx]";
                $('.govnomer').mask('Z999ZZ 999');
                $.mask.definitions['W'] = "[XTCPHMKEBA]";
                $('.ob_type').mask('WWW');
                $('.op_nemer').mask('99999999999999');
                $('.pt_type').mask('99 99');
                $('.pt_nomer').mask('999999');
    </script>
    <script>
        $(document).ready(function(){
            tbl = $('table').DataTable();

            $('.dt-buttons').append('<a class="btn buttons-select-all btn-primary" tabindex="0" aria-controls="DataTables_Table_0" href="#" onclick = "$(\'#dt_columns_custom_hide\').modal(\'show\')"><span>Видимость кoлонок 2</span></a>');
           


            $('thead tr th').each(function(index){
                $('#sortir2').append('<option data-id="'+index+' ">'+$(this).text()+'</option>');
            });
            /*
            $('option').on('dblclick',function($this){
                parent = $(this).parent();
                newPar = parent.attr('id') == 'sortir1' ? 'sortir2' : 'sortir1';
                $(this).detach().appendTo('#'+newPar);
                var column = tbl.column( $(this).attr('data-id') );
                column.visible( ! column.visible() );
                //console.log($(this).attr('data-id')+ ' : '+column.visible());
            });
            
            */


            $('#sortir1').on('dblclick','option',function($this){
                parent = $(this).parent();
                newPar = parent.attr('id') == 'sortir1' ? 'sortir2' : 'sortir1';
                $(this).detach().appendTo('#'+newPar);
                var column = tbl.column( $(this).attr('data-id') );
            ...
            $('#sortir2').on('dblclick','option', function($this){
                parent = $(this).parent();
                newPar = parent.attr('id') == 'sortir1' ? 'sortir2' : 'sortir1';
                $(this).detach().appendTo('#'+newPar);
                var column = tbl.column( $(this).attr('data-id') );
            ...
            $('#dt_custom_hide_allright').on('click',function(){
                /*
                $('thead th').each(function(index){
                    $('#sortir2').append('<option data-id="'+index+'">'+$(this).text()+'</option>');
                    $('#sortir1').find('option').remove();
                    var column = tbl.column(index);
                    column.visible(true);   
                });*/
                $('#sortir1 option').each(function(index){
                    $('#sortir2').append('<option data-id="'+index+'">'+$(this).text()+'</option>');
            ...
            $('#dt_custom_hide_allleft').on('click',function(){
                /*
                $('thead th').each(function(index){
                    $('#sortir2').find('option').remove();
                    $('#sortir1').append('<option data-id="'+index+'">'+$(this).text()+'</option>');
                    var column = tbl.column(index);
                    column.visible(false);    
                });*/
                $('#sortir2 option').each(function(index){
                    $('#sortir1').append('<option data-id="'+index+'">'+$(this).text()+'</option>');
                    tbl.column(index).visible(false);
                    $(this).remove();
                });
            });

            $('#dt_custom_hide_oncer').on('click',function(){
                //console.log('Left');
                //console.log($('#sortir2 option:selected').text());
                id = $('#sortir2 option:selected').attr('data-id');
                $('#sortir2 option:selected').detach().appendTo('#sortir1');
                tbl.column(id).visible(false);

            }); 

            $('#dt_custom_hide_oncel').on('click',function(){
                //console.log('right');
                //console.log($('#sortir1 option:selected').text());
                id = $('#sortir1 option:selected').attr('data-id');
                $('#sortir1 option:selected').detach().appendTo('#sortir2');
                tbl.column(id).visible(true);
            });
        });
    </script>
    <div class="modal fade" id="dt_columns_custom_hide" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Сортировка списка</h5>
      ...
      <div class="modal-body">
        <div class = "row">
            <div class = "col-5">
                <p><strong>Скрытые</strong></p>
                <select name="select" class = "form-control" id = "sortir1" size="5" style = "width: 100% !important; height: 150px !important;">

                </select>
            </div>
            
            <div class = "col-2">
                <p><a class = "btn btn-warning" id = "dt_custom_hide_oncel">-></a></p>
                <p><a class = "btn btn-warning" id = "dt_custom_hide_oncer"><-</a></p>
                <p><a class = "btn btn-warning" id = "dt_custom_hide_allright">->|</a></p>
                <p><a class = "btn btn-warning" id = "dt_custom_hide_allleft">|<-</a></p>
            </div>
            <div class = "col-5">
                <p><strong>Видимые</strong></p>
                <select name="select2" class = "form-control" id = "sortir2" size = "5" style = "width: 100% !important; height: 150px !important;">
