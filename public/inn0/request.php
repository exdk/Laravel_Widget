...
if ($_POST['check']){
	if ($_POST['type'] == "inn") $response_check = json_decode(file_get_contents('https://htmlweb.ru/json/validator/inn/'.$_POST['inn_ogrn']));
	if ($_POST['type'] == "ogrn") $response_check = json_decode(file_get_contents('https://htmlweb.ru/json/validator/ogrn/'.$_POST['inn_ogrn']));
	if ($response_check->status != 200) {
		echo json_encode(["status" => 0, "response" => $response_check->error]);
		exit;
	} 
	...
	foreach ($result->suggestions as $value_api){
		...
		if (!empty($tmp_array)) {
			...
			foreach ($value_api->data->founders as $value_founders){
				...
				$output .= valid_empty("&emsp;&emsp;ФИО учредителя (для физлиц): ", $value_founders->fio->surname. " ".$value_founders->fio->name." ".$value_founders->fio->patronymic, "row", "");
				...
				$output .= valid_empty("&emsp;&emsp;&emsp;Тип значения: ", $data_founders_share[$value_founders->share->type], "row", "");
				$output .= valid_empty("&emsp;&emsp;&emsp;Значение: ", $value_founders->share->value, "row", "");
		...
		if (!empty($tmp_array)) {
			...
			foreach ($value_api->data->phones as $value_phones){
				$output .= valid_empty("&emsp;Контактное лицо: ", (isset($value_phones->data->contact) ? $value_phones->data->contact : ""), "row", "");
		...
		if (!empty($tmp_array)) {
			...
			foreach ($value_api->data->emails as $value_emails){
				...
				$output .= valid_empty("&emsp;&emsp;домен (то, что после (то, что после @): ", (isset($value_emails->data->domain) ? $value_emails->data->domain : ""), "row", "");
			}
		}
	}
