...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.docTtn.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.doc-ttns.update", [$docTtn->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_list_product') ? 'is-invalid' : '' }}" name="id_list_product_id" id="id_list_product_id">
                    @foreach($id_list_products as $id => $entry)
                        <option value="{{ $id }}" {{ (old('id_list_product_id') ? old('id_list_product_id') : $docTtn->id_list_product->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('otpravitel') ? 'is-invalid' : '' }}" name="otpravitel_id" id="otpravitel_id">
                    @foreach($otpravitels as $id => $entry)
                        <option value="{{ $id }}" {{ (old('otpravitel_id') ? old('otpravitel_id') : $docTtn->otpravitel->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('poluchatel') ? 'is-invalid' : '' }}" name="poluchatel_id" id="poluchatel_id">
                    @foreach($poluchatels as $id => $entry)
                        <option value="{{ $id }}" {{ (old('poluchatel_id') ? old('poluchatel_id') : $docTtn->poluchatel->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
