...
class SendRFQEndMessage extends Command
{
   /**
    ...
    * @var string
    */
   protected $signature = 'send_rfq_end_alerts:send';
   /**
    * The console command description.
    *
    * @var string
    */
   protected $description = 'Sending rfq mails and alerts on finish date';
   /**
   ...
   {
	   $rfqs = Rfq::where("finish", ">=", Carbon::now()->format("Y-m-d H:i:00"))->where("finish", "<=", Carbon::now()->format("Y-m-d H:i:59"))->get();
	   foreach($rfqs as $rfq)
	   {
			$company_emails = [];
			$company_users = [];
			...
			{
				$ans = $SMTPBZController->send_mail("Уведомление об окончании RFQ", $ce, $html);
			}
			$alert = UserAlert::create(["alert_text" => "Окончание RFQ", "alert_link" => "/admin/rfqs/".$rfq->id]);
			$user_user_alert = [];
			/*Отправка сотрудникам*/
			foreach($selected_transport_companies_employers as $e)
			{
				if($e->notifications_settings->rfq_end == 1)
				{
					$ans = $SMTPBZController->send_mail("Уведомление об окончании RFQ", $e->email, $html);
					$user_user_alert[] = $e->id;
				}
			}
			foreach($company_users as $cu)
			{
				if($cu->notifications_settings->rfq_end == 1)
				{
