...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.partnerBb.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.partner-bbs.update", [$partnerBb->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\PartnerBb::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $partnerBb->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('partner') ? 'is-invalid' : '' }}" name="partner_id" id="partner_id">
                    @foreach($partners as $id => $entry)
                        <option value="{{ $id }}" {{ (old('partner_id') ? old('partner_id') : $partnerBb->partner->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('typedogovor') ? 'is-invalid' : '' }}" name="typedogovor" id="typedogovor" required>
                    ...
                    @foreach(App\Models\PartnerBb::TYPEDOGOVOR_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('typedogovor', $partnerBb->typedogovor) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('dogovor_number') ? 'is-invalid' : '' }}" type="text" name="dogovor_number" id="dogovor_number" value="{{ old('dogovor_number', $partnerBb->dogovor_number) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('dogovor_start') ? 'is-invalid' : '' }}" type="text" name="dogovor_start" id="dogovor_start" value="{{ old('dogovor_start', $partnerBb->dogovor_start) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('dogovor_end') ? 'is-invalid' : '' }}" type="text" name="dogovor_end" id="dogovor_end" value="{{ old('dogovor_end', $partnerBb->dogovor_end) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('valuta') ? 'is-invalid' : '' }}" name="valuta_id" id="valuta_id">
                    @foreach($valutas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('valuta_id') ? old('valuta_id') : $partnerBb->valuta->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $partnerBb->description) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('contactperson') ? 'is-invalid' : '' }}" type="text" name="contactperson" id="contactperson" value="{{ old('contactperson', $partnerBb->contactperson) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('telefon') ? 'is-invalid' : '' }}" type="text" name="telefon" id="telefon" value="{{ old('telefon', $partnerBb->telefon) }}">
