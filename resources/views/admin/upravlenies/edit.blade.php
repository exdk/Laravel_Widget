...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.upravlenie.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.upravlenies.update", [$upravlenie->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('mainauto') ? 'is-invalid' : '' }}" name="mainauto_id" id="mainauto_id" required>
                    @foreach($mainautos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('mainauto_id') ? old('mainauto_id') : $upravlenie->mainauto->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('prizep') ? 'is-invalid' : '' }}" name="prizep_id" id="prizep_id">
                    @foreach($prizeps as $id => $entry)
                        <option value="{{ $id }}" {{ (old('prizep_id') ? old('prizep_id') : $upravlenie->prizep->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('driver') ? 'is-invalid' : '' }}" name="driver_id" id="driver_id">
                    @foreach($drivers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('driver_id') ? old('driver_id') : $upravlenie->driver->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('driver_2') ? 'is-invalid' : '' }}" name="driver_2_id" id="driver_2_id">
                    @foreach($driver_2s as $id => $entry)
                        <option value="{{ $id }}" {{ (old('driver_2_id') ? old('driver_2_id') : $upravlenie->driver_2->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('datestart') ? 'is-invalid' : '' }}" type="text" name="datestart" id="datestart" value="{{ old('datestart', $upravlenie->datestart) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_fin') ? 'is-invalid' : '' }}" type="text" name="date_fin" id="date_fin" value="{{ old('date_fin', $upravlenie->date_fin) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('timetart') ? 'is-invalid' : '' }}" type="text" name="timetart" id="timetart" value="{{ old('timetart', $upravlenie->timetart) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('time_fin') ? 'is-invalid' : '' }}" type="text" name="time_fin" id="time_fin" value="{{ old('time_fin', $upravlenie->time_fin) }}">
